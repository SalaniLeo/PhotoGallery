...
export async function get_posts() {
    const response = await fetch('http://127.0.0.1/api/get_posts', {
      method: 'GET',
      headers: {
...
export async function delete_post(title, description, path, time) {
    const response = await fetch('http://127.0.0.1/api/delete_post', {
      method: 'POST',
...
export async function upload_post(title, description, file) {
  console.log(file.files[0])
    const response = await fetch('http://127.0.0.1/api/upload_post', {
      method: 'POST',
